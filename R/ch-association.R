...
###################################################
library(tidyverse)
titanic <- read_csv("~/RHome/COMCAST/data/titanic-imp1.csv")
...
titanic <- titanic %>% mutate(
                            pass_class = as.factor(pclass, levels=NULL, labels = c"1st", "2nd", "3rd"), ordered = is.ordered()),
                            survived = as.factor(survived, levels=NULL, labels = c("Dead", "Alive")),
                            name = parse_character(name, levels=NULL),
                            gender = parse_factor(sex, levels=NULL),
                            ticket = parse_factor(ticket, levels=NULL),
                            cabin = as.factor(cabin, levels=NULL),
                            embarked = parse_factor(embarked, levels=NULL),
                            dest = parse_factor(home.dest, levels=NULL)
)
